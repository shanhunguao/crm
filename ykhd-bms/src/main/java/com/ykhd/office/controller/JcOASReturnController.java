...
@RestController
@RequestMapping("/jc_oas_return")
public class JcOASReturnController extends BaseController {
	...
	public Object list(OASReturnCondition condition) {
		return jcOASReturnService.getListByPage(condition);
	}
	...
	public void exportExcel(OASReturnCondition condition, HttpServletResponse response) {
		jcOASReturnService.exportExcel(condition, response);
	}
	...
	public Object totalReturn(String schedule) {
		...
		return jcOASReturnService.totalReturn(Stream.of(schedule.split(",")).map(Integer::valueOf).collect(Collectors.toList()));
	}
	...
	public Object add(@Valid OASReturnSubmit submit) {
		return jcOASReturnService.addOASReturn(submit) ? success(null) : failure(save_failure);
	}
	...
	public Object confirm(@PathVariable String id, MultipartFile[] files, String date, String remark) {
		...
		return jcOASReturnService.confirm(Stream.of(id.split(",")).map(Integer::valueOf).collect(Collectors.toList()), files, date, remark);
	}
	...
	public Object refuse(@PathVariable String id) {
		return jcOASReturnService.refuse(Stream.of(id.split(",")).map(Integer::valueOf).collect(Collectors.toList()));
	}
	...
	public Object check(@PathVariable Integer id, String filePath, MultipartFile[] files, String date, String remark) {
		...
		return jcOASReturnService.check(id, oldPath, files, date, remark) ? success(null) : failure(update_failure);
	}
	...
	public Object checkBatch(@PathVariable String id, MultipartFile[] files, String date) {
		return jcOASReturnService.checkBatch(Stream.of(id.split(",")).map(Integer::valueOf).collect(Collectors.toList()), files, date);
	}
	...
	public Object checkFailure(@PathVariable String id, String reason) {
		return jcOASReturnService.checkFailure(Stream.of(id.split(",")).map(Integer::valueOf).collect(Collectors.toList()), reason);
	}
