...
public interface PaymentAppMapper extends BaseMapper<PaymentApp> {
	 ...
	 */
	@Select("select ifnull(sum(amount), 0) from wm_payment_app where schedule = ${schedule} and type = ${type} and state in (${states})")
	BigDecimal totalByPayAndApp(Integer schedule, Integer type, String states);
	
	/**
	 * 排期历史已支付的总金额
	 */
	@Select("select schedule as id, sum(amount) as amount from wm_payment_app where type = ${type} and state = ${state} and schedule in (${scheduleIds}) group by schedule")
	List<IdDecimal> totalPayByScheduleIds(@Param("type") Integer type,@Param("state") Integer state,@Param("scheduleIds") String scheduleIds);
	
	/**
	 * 是否存在关于排期的有效支付申请
	 */
	@Select("select count(1) from wm_payment_app where schedule = ${schedule} and state in (${states})")
	int existPaymentApp(Integer schedule, String states);
	
	/**
	 * 根据公众号的名称或微信号查询 支付申请id
	 */
	@Select("select p.id from wm_payment_app p, wm_oa_schedule oas, wm_office_account oa where p.schedule = oas.id and oas.office_account = oa.id "
	...
	@Select("select p.fapiao as fapiao, p.amount as amount, oas.creator as id, oas.advantage as advantage "
			+ "from wm_payment_app p "
			+ "left join wm_oa_schedule oas on p.schedule = oas.id "
