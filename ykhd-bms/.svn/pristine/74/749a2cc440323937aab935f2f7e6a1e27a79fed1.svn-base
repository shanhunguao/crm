...
import com.ykhd.office.repository.JcOAScheduleMapper;
...
public class JcOAScheduleService extends ServiceImpl<BaseMapper<JcOASchedule>, JcOASchedule> implements IJcOAScheduleService {
	...
	@Autowired
	private JcOAScheduleMapper jcOAScheduleMapper;
	@Autowired
	...
	public boolean confirm_create(Integer oaScheduleId, Integer oaFapiao, Integer oaFapiaoType) {
		...
		schedule.setReturnTime(schedule.getCreateTime());
	...
	public PageHelpers<OAScheduleDto> getListByPage(OAScheduleCondition condition) {
		...
		QueryWrapper<JcOASchedule> queryWrapper = new QueryWrapper<>();
		// 【查询限制】
		LoginCache managerInfo = BaseController.getManagerInfo();
		...
		if (StringUtils.hasText(condition.getRemark())) {
			List<Integer> scheduleIds = jcPaymentAppService.queryOascheIdByPayRemark(condition.getRemark().trim());
			List<Integer> scheduleIds2 = jcOASReturnService.queryOascheIdByReturnRemark(condition.getRemark().trim());
		...
		if (condition.getDateType() != null) {
			...
			if (date_type == 4) { //付款时间
				QueryWrapper<JcPaymentApp> payQuery = new QueryWrapper<>();
				...
				List<Integer> scheIds = jcPaymentAppService.list(payQuery).stream().map(v -> v.getSchedule()).collect(Collectors.toList());
				if (scheIds.isEmpty())
					return new PageHelpers<OAScheduleDto>(condition.getPage(), condition.getSize(), 0, Collections.emptyList(), Collections.emptyMap());
				else
		...
		if (condition.getHaveOaFapiao() != null) {
			if (condition.getHaveOaFapiao() == 0) { //已开进项票
				List<Integer> scheIds = jcPaymentAppService.havaFapiao();
				if (scheIds.isEmpty())
					return new PageHelpers<OAScheduleDto>(condition.getPage(), condition.getSize(), 0, Collections.emptyList(), Collections.emptyMap());
				queryWrapper.in("id", scheIds);
			} else if (condition.getHaveOaFapiao() == 1) { //已支付未开进项票
				List<Integer> scheIds = jcPaymentAppService.noHavaFapiao();
				if (scheIds.isEmpty())
					return new PageHelpers<OAScheduleDto>(condition.getPage(), condition.getSize(), 0, Collections.emptyList(), Collections.emptyMap());
				queryWrapper.in("id", scheIds);
			}
		...
		if (condition.getHaveCustFapiao() != null) {
			if (condition.getHaveCustFapiao() == 0) { //已开销项票
				List<Integer> scheIds = jcOASReturnService.makeFapiao();
				if (scheIds.isEmpty())
					return new PageHelpers<OAScheduleDto>(condition.getPage(), condition.getSize(), 0, Collections.emptyList(), Collections.emptyMap());
				queryWrapper.in("id", scheIds);
			} else if (condition.getHaveCustFapiao() == 1) { //未开销项票
				List<Integer> scheIds = jcOASReturnService.noMakeFapiao();
				if (scheIds.isEmpty())
					return new PageHelpers<OAScheduleDto>(condition.getPage(), condition.getSize(), 0, Collections.emptyList(), Collections.emptyMap());
				queryWrapper.in("id", scheIds);
			}
		...
		IPage<JcOASchedule> iPage = page(new Page<>(condition.getPage(), condition.getSize()), queryWrapper);
		List<JcOASchedule> records = iPage.getRecords();
		if (records.isEmpty()) {
			...
			Map<Integer, String> payRemarkMap = jcPaymentAppService.queryPayRemarkByScheId(idSet);
			Map<Integer, String> returnRemarkMap = jcOASReturnService.queryReturnRemarkByScheId(idSet);
			Map<Integer, BigDecimal> totalPayMap = jcPaymentAppService.queryTotalPayByScheId(
					list.stream().filter(v -> v.getIsPay() == null).map(v -> v.getId()).collect(Collectors.toSet()));
			Map<Integer, BigDecimal> totalReturnMap = jcOASReturnService.queryTotalReturnByScheId(
			...
			if (condition.getHaveOaFapiao() == null)
				haveOaFapiaoMap = jcPaymentAppService.ifHaveFapiao(idSet);
			Map<Integer, Boolean> haveOaFapiaoMap2 = haveOaFapiaoMap; // final
			Map<Integer, Boolean> haveCustFapiaoMap = null;
			if (condition.getHaveCustFapiao() == null)
				haveCustFapiaoMap = jcOASReturnService.ifMakeFapiao(idSet);
			...
			// 显示列表数据汇总
			List<JcOASchedule> all = list(queryWrapper.select("execute_price", "cost_price", "sell_expense", "channel_expense", "cust_fapiao", "oa_fapiao", "oa_fapiao_type"));
	...
	public void exportExcel(OAScheduleCondition condition, HttpServletResponse response) {
		...
		if (StringUtils.hasText(condition.getRemark())) {
			List<Integer> scheduleIds = jcPaymentAppService.queryOascheIdByPayRemark(condition.getRemark());
			List<Integer> scheduleIds2 = jcOASReturnService.queryOascheIdByReturnRemark(condition.getRemark().trim());
			scheduleIds.addAll(scheduleIds2);
			List<Integer> all = scheduleIds.stream().distinct().collect(Collectors.toList());
			if (all.isEmpty())
		...
		if (StringUtils.hasText(condition.getSort())) {
			...
			queryWrapper.orderByDesc("id");
		List<JcOASchedule> records = list(queryWrapper);
		if (records.isEmpty())
			list = Collections.emptyList();
		else {
			list = new ArrayList<>(records.size());
...
//	}
	...
	public void checkReturnFinish(Integer id, BigDecimal total, Date return_time) {
		JcOASchedule schedule = getOne(new QueryWrapper<JcOASchedule>().select("id", "execute_price", "sell_expense", "is_pay").eq("id", id));
		if (schedule.getExecutePrice().compareTo(total) == 0)
			Assert.state(update(new UpdateWrapper<JcOASchedule>().eq("id", id).set("is_return_money", 1).set("return_time", return_time)), "回款检查完成：已回款状态修改失败");
		KeyValuePair<BigDecimal, BigDecimal> cost_sell = jcPaymentAppService.totalPayByScheId(id);
		...
		if (schedule.getIsPay() != null && schedule.getIsPay() == 1)
			if (schedule.getSellExpense() == null || schedule.getSellExpense().doubleValue() == sellTotal.doubleValue())
				Assert.state(update(new UpdateWrapper<JcOASchedule>().eq("id", id).set("state", State4OASchedule.已完成.code())), "回款检查完成：已完成状态修改失败");
	}

	@Override
	public void checkPayFinish(Integer id) {
		JcOASchedule schedule = getOne(new QueryWrapper<JcOASchedule>().select("cost_price", "sell_expense", "is_pay", "is_return_money").eq("id", id));
		if (schedule.getIsPay() == null) { //未支付
			...
			if (schedule.getCostPrice().doubleValue() == costTotal.doubleValue()) {
				Assert.state(update(new UpdateWrapper<JcOASchedule>().eq("id", id).set("is_pay", 1)), "支付检查完成：已支付状态修改失败");
				if (schedule.getSellExpense() == null || schedule.getSellExpense().doubleValue() == sellTotal.doubleValue())
					if (schedule.getIsReturnMoney() != null && schedule.getIsReturnMoney() == 1)
						Assert.state(update(new UpdateWrapper<JcOASchedule>().eq("id", id).set("state", State4OASchedule.已完成.code())), "支付检查完成：已完成状态修改失败");
			}
		} else { // 最后复核的销售费用支付
			KeyValuePair<BigDecimal, BigDecimal> cost_sell = jcPaymentAppService.totalPayByScheId(id);
			BigDecimal sellTotal = cost_sell.getValue();
...
//	}
	...
	public Map<Integer, OASimpleInfo> queryIdOaNameWechatBrand(Collection<Integer> idSet) {
		return idSet.isEmpty() ? Collections.emptyMap() : 
			jcOAScheduleMapper.queryIdNameWechatBrand(idSet.stream().map(String::valueOf).collect(Collectors.joining(",")))
	...
	public Map<Integer, OAScheduleInfo> info4PaymentAppList(Collection<Integer> collection) {
		return collection.isEmpty() ? Collections.emptyMap() : 
			jcOAScheduleMapper.info4PaymentAppList(collection.stream().map(String::valueOf).collect(Collectors.joining(",")))
	...
	public Map<Integer, OAScheduleInfo2> info4OASReturnList(Collection<Integer> collection) {
		return collection.isEmpty() ? Collections.emptyMap() : 
			jcOAScheduleMapper.info4OASReturnList(collection.stream().map(String::valueOf).collect(Collectors.joining(",")))
			.stream().collect(Collectors.toMap(OAScheduleInfo2::getId, v -> v));
	}

	@Override
	public List<Integer> queryIdByCreator(Integer creator) {
		List<JcOASchedule> list = list(new QueryWrapper<JcOASchedule>().select("id").eq("creator", creator));
		return list.stream().map(JcOASchedule::getId).collect(Collectors.toList());
	}
	
	@Override
	public List<Integer> queryIdByOAName(String name) {
		return jcOAScheduleMapper.queryIdByOAName(name);
	}

	@Override
	public List<Integer> queryIdByCustomer(String name) {
		return jcOAScheduleMapper.queryIdByCustomer(name);
	}

	@Override
	public Map<Integer, Integer> queryScheduleCount(Collection<Integer> collection) {
		return collection.isEmpty() ? Collections.emptyMap() : 
			jcOAScheduleMapper.queryScheduleCount(collection.stream().map(String::valueOf).collect(Collectors.joining(",")), 
...
//		return collection.isEmpty() ? Collections.emptyMap() : 
//			list(new QueryWrapper<JcOASchedule>().select("id", "medium_name").in("id", collection))
//			.stream().collect(Collectors.toMap(JcOASchedule::getId, JcOASchedule::getMediumName));
//	}

	@Override
	public void removeIsPay(Integer id) {
		JcOASchedule oASchedule = getOne(new QueryWrapper<JcOASchedule>().select("state", "is_pay").eq("id", id));
		Assert.notNull(oASchedule, "未知排期");
		if (oASchedule.getIsPay() != null) {
			if (oASchedule.getState().intValue() == State4OASchedule.已完成.code())
				Assert.state(update(new UpdateWrapper<JcOASchedule>().eq("id", id).set("is_pay", null).set("state", State4OASchedule.待结算.code())), "回退已支付：已完成_失败");
			else
				Assert.state(update(new UpdateWrapper<JcOASchedule>().eq("id", id).set("is_pay", null)), "回退已支付：失败");
		}
	}
}
